...
Primary command:
- `uv run ai-tools repo submit --json $ARGUMENTS`
...
- If `uv run ai-tools repo submit` is unavailable, try `uv run ai-tools repo check run --json` to provide pre-submit validation context.
