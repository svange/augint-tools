...
from augint_tools.cli.commands import new as new_cmd
...
class TestNewPrereqCheck:
    ...
    def test_missing_prereq_shows_install_hint(self, monkeypatch):
        monkeypatch.setattr(new_cmd.shutil, "which", lambda _tool: None)
        runner = CliRunner()
        result = runner.invoke(cli, ["new"], input="2\n")
    ...
    def test_every_prereq_has_install_hint(self):
        all_prereqs = {p for pt in new_cmd.PROJECT_TYPES for p in pt.prereqs}
        missing_hints = all_prereqs - set(new_cmd._INSTALL_HINTS.keys())
